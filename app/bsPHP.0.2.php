...
class bs{
	static private function DEFINE(){
		...
		define( 'SYS_PATH', 'SYS' );
		define( 'SYS_CONTROLLER', APP.'sys/controller/' );
		define( 'SYS_VIEW', APP.'sys/view/' );
		define( 'LIB_PATH', 'LIB' );
		define( 'LIB_CONTROLLER', APP.'lib/controller/' );
		define( 'LIB_VIEW', APP.'lib/view/' );
		//application
	...
	static private function route( $uri = NULL ){
		...
		if( $uri == '/' || empty($uri) ){
			...
			if( $uri[0] == SYS_PATH ){
				if( @BSSYS === TRUE ){
					$dir = substr( SYS_CONTROLLER, 0, -1 );
					self::$viewMode = 'sys';
				}else self::response( HTTP_NOT_FOUND, 'Controller Not Found :'.$_SERVER['REQUEST_URI'] );
			}else if( $uri[0] == LIB_PATH ){
				$dir = substr( LIB_CONTROLLER, 0, -1 );
				self::$viewMode = 'lib';
			}else{
				$dir = substr( CONTROLLER, 0, -1 );
				$i = 0;
			}
			for( $j = count($uri) ; $i < $j && is_dir( $dir.'/'.$uri[$i] ) ; ) $dir .= '/'.$uri[$i++];
	...
	static function view( $key, $cache = TRUE ){
		...
		eval( '?>'.self::appFile( $k.$key, $v.$key.EXT, $cache ).'<?php ' );
	}
	//err
	...
	static private function queryAdd($key){
		return !isset(self::$sql[$key]) && !self::sqlAdd($key) ? FALSE : TRUE;
	}
	...
	static function queryData( $key ){
		if( !self::queryAdd($key) ) return FALSE;
		return count(self::$sqlInfo[$key]) > 0 ? self::$sqlInfo[$key] : FALSE;
	}
	static function query( $key, $data = NULL ){
		if( !self::queryAdd($key) ) return self::err( 5000, $key );
